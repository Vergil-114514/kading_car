...
// 备注信息
//-------------------------------------------------------------------------------------------------------------------
static uint8 gnss_nmea_checksum_valid (const uint8 *buffer)
{
    uint8 check_buffer[5] = {'0', 'x', 0x00, 0x00, 0x00};
    uint8 bbc_xor_origin;
    ...
    // 接收回调会保证缓冲区以 \0 结束，因此 strchr 不会越界
    checksum_position = strchr((const char *)buffer, '*');
    if((NULL == checksum_position) ||
       ('\0' == checksum_position[1]) || ('\0' == checksum_position[2]))
    {
        return 0;
    }

    check_buffer[2] = (uint8)checksum_position[1];
    check_buffer[3] = (uint8)checksum_position[2];
    bbc_xor_origin = (uint8)func_str_to_hex((char *)check_buffer);

    for(data_len = 1; data_len < GNSS_BUFFER_SIZE; data_len ++)
    {
        if('*' == buffer[data_len])
        {
            return (uint8)(bbc_xor_calculation == bbc_xor_origin);
        }
        if('\0' == buffer[data_len])
        {
            break;
        }
        bbc_xor_calculation ^= buffer[data_len];
    }
    return 0;
}
...
uint8 gnss_data_parse (void)
{
    ...
    do
    {
        if(GPS_STATE_RECEIVED == gnss_rmc_state)
        {
            ...
            if(!gnss_nmea_checksum_valid(gps_rmc_buffer))
            {
                // 数据校验失败
                // 必须退出 PARSING，否则后续 RMC 帧会被接收回调永久丢弃
        ...
        if(GPS_STATE_RECEIVED == gnss_gga_state)
        {
            ...
            if(!gnss_nmea_checksum_valid(gps_gga_buffer))
            {
                // 数据校验失败
        ...
        if(GPS_STATE_RECEIVED == gnss_ths_state)
        {
            ...
            if(!gnss_nmea_checksum_valid(gps_ths_buffer))
            {
                // 数据校验失败
...
void gnss_uart_callback (void)
{
    ...
    if(gnss_state)
    {
        ...
        while(uart_query_byte(GNSS_UART, &dat))
        {
            ...
            // 在排空 UART FIFO 的过程中逐句处理，避免多句 NMEA 被粘在一起
            if('\n' == dat)
            {
                // 组合导航只需要 RMC；前6字节用于识别语句类型
                temp_length = 6;
                fifo_read_buffer(&gnss_receiver_fifo, temp_gps, &temp_length, FIFO_READ_ONLY);

                if((6 == temp_length) &&
                   (0 == strncmp((char *)&temp_gps[3], "RMC", 3)))
                {
                    // 如果没有在解析数据则用最新一帧覆盖待解析缓冲区
...
void gnss_init (gps_device_enum gps_device)
{
    ...
    const uint8 close_gga[]     = {0xF1, 0xD9, 0x06, 0x01, 0x03, 0x00, 0xF0, 0x00, 0x00, 0xFA, 0x09};
    ...
    if((TAU1201 == gps_device) || (GN42A == gps_device))
    {
        ...
        system_delay_ms(50);    
        uart_write_buffer(GNSS_UART, (uint8 *)close_gga, sizeof(close_gga));    // 组合导航仅保留RMC语句
        system_delay_ms(50);
