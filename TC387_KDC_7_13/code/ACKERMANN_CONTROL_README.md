...
当前按 AB 正交模式初始化。若编码器实际配置成“脉冲 + 方向”模式，应把 `ACKERMANN_PORT_ENCODER_USE_QUADRATURE` 改为 `0U`。

仍需确认或标定：

- 修改 `ACKERMANN_PORT_REAR_ENCODER_CPR`；
- 如果轮速符号相反，修改对应的 `ENCODER_SIGN`；
- 如果正 PWM 使车辆后退，修改对应的 `MOTOR_SIGN`。

这里的 CPR 必须是“车轮转一整圈时 TC387 实际累计的计数”，包括减速比和 AB 相四倍频。例如电机编码器每转 500 线、四倍频、减速比 10:1，则车轮每圈计数通常为：

```text
500 × 4 × 10 = 20000
```

最可靠的方法是架起车辆，将车轮人工旋转一整圈，直接读取累计计数。
